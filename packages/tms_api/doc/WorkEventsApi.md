...
[**listEmployeeWorkEvents**](WorkEventsApi.md#listemployeeworkevents) | **GET** /user-management/v1/employees/{employeeId}/workEvents | List Employees Time Entries.
...
> BuiltList<WorkEvent> listEmployeeWorkEvents(employeeId, after, before, first, max)
...
try {
    final response = api.listEmployeeWorkEvents(employeeId, after, before, first, max);
    print(response);
} catch on DioException (e) {
